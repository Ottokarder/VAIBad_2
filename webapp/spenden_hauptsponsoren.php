...
// Gespeicherte Hauptsponsor-Spenden-Ergebnisse abrufen (mit Namen).
$sql = "
    SELECT st.hauptsponsor_id,
           st.schwimmer_id,
           ...
           st.erstelldatum,
           h.name AS hauptsponsor_name,
           h.`limit` AS hauptsponsor_limit,
           CONCAT(sw.vorname, ' ', sw.nachname) AS schwimmer_name,
           sw.startnummer
    FROM spenden_hauptsponsoren st
    JOIN Hauptsponsoren h ON st.hauptsponsor_id = h.id
    JOIN Schwimmer sw ON st.schwimmer_id = sw.id
    ORDER BY h.name, sw.startnummer, sw.nachname, sw.vorname
";
$result = $conn->query($sql);

// Ergebnisse zwischenspeichern und nach Hauptsponsor gruppieren.
$gruppen = [];
$gesamt_summe = 0.0;
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $hid = $row['hauptsponsor_id'];
        if (!isset($gruppen[$hid])) {
            $gruppen[$hid] = [
                'info'    => ['hauptsponsor_name' => $row['hauptsponsor_name'], 'hauptsponsor_limit' => $row['hauptsponsor_limit']],
                'zeilen'  => [],
                'sum_gesamt'    => 0.0,
                'sum_gedeckelt' => 0.0,
            ];
        }
        $gruppen[$hid]['zeilen'][] = $row;
        $gruppen[$hid]['sum_gesamt']    += (float)$row['spendenbetrag_gesamt'];
        $gruppen[$hid]['sum_gedeckelt'] += (float)$row['spendenbetrag_gedeckelt'];
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $dateiname = 'hauptsponsor_spendenbetr_' . date('Y-m-d_His') . '.csv';
    ...
    foreach ($gruppen as $hid => $g) {
        ...
        // Hauptsponsor-Zwischensumme
        fputcsv($out, [
            $g['info']['hauptsponsor_name'] . ' - Summe', '', '',
            '', '',
...
<div class="container">
    ...
    <?php if (!empty($gruppen)): ?>
        <?php foreach ($gruppen as $hid => $g): ?>
            <h2 style="margin-top: 2rem;">
                <?php echo htmlspecialchars($g['info']['hauptsponsor_name']); ?>
                <span style="font-size:.9rem; font-weight:normal; color:#666;">
                    (Limit: <?php echo ($g['info']['hauptsponsor_limit'] !== null) ? htmlspecialchars($g['info']['hauptsponsor_limit']) . ' €' : 'kein Limit'; ?>)
                </span>
            </h2>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr style="font-weight: bold; background-color: #f0f0f0;">
                            <td colspan="4">Hauptsponsor-Summe</td>
        ...
        <!-- Gesamtsumme über alle Hauptsponsoren -->
